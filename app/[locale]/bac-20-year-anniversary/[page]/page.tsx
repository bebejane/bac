...
export const AnniversaryPagination = ({ pages }: { pages: AnniversaryPageRecord[] }) => {
	...
	return (
		<nav className={cn(s.pagination, 'background-palette-animation')}>
			<Link href={`/bac-20-year-anniversary/${prev.slug}`}>{t('previous')}</Link>
			<Link href={`/bac-20-year-anniversary`} className={s.logo}>
				<img src='/images/anniversary-logo-20.svg' alt='BAC Logo' />
				<span>Översikt</span>
			</Link>
			<Link href={`/bac-20-year-anniversary/${next.slug}`}>{t('next')}</Link>
		</nav>
	);
};
...
export const getStaticProps = withGlobalProps(
	{ queries: [AllAnniversaryPagesDocument] },
	async ({ props, revalidate, context }: any) => {
		const slug = context.params.page;
		const { anniversaryPage } = await apiQuery(AnniversaryPageDocument, {
			variables: { slug, locale: context.locale },
			preview: context.preview,
		});

		if (!anniversaryPage) return { notFound: true, revalidate };
